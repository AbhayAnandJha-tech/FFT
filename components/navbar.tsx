...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="absolute bottom-0 left-0 h-0.5 bg-[#ffd700] w-[var(--scroll-progress)] transition-width duration-100"></div>
        <div className="max-w-screen-xl mx-auto px-5 py-3 flex justify-between items-center relative">
          ...
          <div className="hidden md:flex gap-8 items-center">
            {navItems.map((item, index) => (
              ...
              >
                {item.dropdown ? (
                  <>
                    <motion.button className="text-[#ffa500] font-medium py-2 px-4 rounded-lg hover:scale-105 transition-all duration-300">
                      {item.label}
                      <motion.span
                  ...
                  <Link
                    to={item.to}
                    className="text-[#ffa500] font-medium py-2 px-4 rounded-lg hover:bg-gray-100"
                  >
                    {item.label}
                  </Link>
                )}
          ...
          <div className="flex items-center gap-4 z-20">
            ...
            {isLoggedIn ? (
              ...
              <motion.button
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-[#ffd700] to-[#ffa500] text-white font-medium hover:scale-105 transition-all duration-300"
            ...
            <motion.button
              className="bg-transparent border-none text-[#ffa500] p-2 hover:scale-110 transition-all duration-300"
              ...
              >
                {navItems.map((item) => (
                  <div key={item.label} className="py-4">
                    {item.dropdown ? (
                      <>
                        <button className="w-full flex justify-between items-center py-2 px-4 text-lg text-[#ffa500]">
                          {item.label}
                          <ChevronDown size={16} />
                        </button>
                        <div className="pl-4">
                      ...
                      <Link
                        to={item.to}
                        className="text-lg text-[#ffa500] py-3 px-4 block hover:bg-gray-100"
                      >
                        {item.label}
                      </Link>
                    )}
